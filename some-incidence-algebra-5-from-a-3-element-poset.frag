...
#info Incidence algebra of Finite poset containing 3 elements over Real Field with 53 bits of precision
...
float[N] product(float u[N], float v[N]) {
    return float[N](1.0*u[0]*v[0], u[1]*v[0] + u[2]*v[1], 1.0*u[2]*v[2], u[3]*v[0] + u[4]*v[3], 1.0*u[4]*v[4]);
}
