...
uniform float CH; slider[0,99,9999999999999]
...
float[N] product(float u[N], float v[N]) {
	u = reduce(u);
	v = reduce(v);
    float w[N] = float[N](u[0]*v[0] + u[1]*v[1] + u[2]*v[2] - u[3]*v[3], u[0]*v[1] + u[1]*v[0] - u[2]*v[3] + u[3]*v[2], u[0]*v[2] + u[1]*v[3] + u[2]*v[0] - u[3]*v[1], u[0]*v[3] + u[1]*v[2] - u[2]*v[1] + u[3]*v[0]);
    return reduce(w);
}
