...
#info 8 dimensional fractal quest on Group algebra of Quaternion group of order 8 as a permutation group over Symbolic Ring
#group Algebraic
...
float[N] product(float u[N], float v[N]) {
    return float[N](8*u[7]*v[7], 8*u[7]*v[7], 8*u[7]*v[7], 8*u[7]*v[7], 8*u[7]*v[7], 8*u[7]*v[7], 8*u[7]*v[7], 8*u[7]*v[7]);
}
...
float[N] antipode(float u[N]) {
    return float[N](u[7], u[7], u[7], u[7], u[7], u[7], u[7], u[7]);
}
