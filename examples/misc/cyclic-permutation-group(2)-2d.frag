...
#info 2 dimensional fractal quest on Group algebra of Cyclic group of order 2 as a permutation group over Symbolic Ring
#group Algebraic
...
uniform int mutationA; slider[0,0,2]
uniform int mutationB; slider[0,0,2]
uniform int mutationC; slider[0,0,2]
uniform int mutationD; slider[0,0,2]
...
float[N] product(float u[N], float v[N]) {
    return float[N](u[0]*v[0] + u[1]*v[1], u[0]*v[1] + u[1]*v[0]);
}


float norm(float u[N]) {
    return pow(pow(abs(u[0]), 2.0) + pow(abs(u[1]), 2.0), 0.5);
}


float[N] antipode(float u[N]) {
    return float[N](u[0], u[1]);
}
