...
#info Clifford Algebra with signature [-3,-3,-3]
...
float[N] product(float u[N], float v[N]) {
    return float[N](u[0]*v[0] - 3.0*u[1]*v[1] - 9.0*u[2]*v[2] + 27.0*u[3]*v[3] - 9.0*u[4]*v[4] - 3.0*u[5]*v[5] - 9.0*u[6]*v[6] - 3.0*u[7]*v[7], u[0]*v[1] + u[1]*v[0] - 3.0*u[2]*v[5] - 9.0*u[3]*v[6] - 3.0*u[4]*v[7] + 3.0*u[5]*v[2] - 9.0*u[6]*v[3] + 3.0*u[7]*v[4], u[0]*v[2] + u[1]*v[5] + u[2]*v[0] - 3.0*u[3]*v[7] + 3.0*u[4]*v[6] - u[5]*v[1] - 3.0*u[6]*v[4] - 3.0*u[7]*v[3], u[0]*v[3] + u[1]*v[6] + u[2]*v[7] + u[3]*v[0] - u[4]*v[5] - u[5]*v[4] + u[6]*v[1] + u[7]*v[2], u[0]*v[4] + u[1]*v[7] - 3.0*u[2]*v[6] + 3.0*u[3]*v[5] + u[4]*v[0] + 3.0*u[5]*v[3] + 3.0*u[6]*v[2] - u[7]*v[1], u[0]*v[5] - 3.0*u[1]*v[2] + 3.0*u[2]*v[1] + 9.0*u[3]*v[4] + 9.0*u[4]*v[3] + u[5]*v[0] - 3.0*u[6]*v[7] + 3.0*u[7]*v[6], u[0]*v[6] - 3.0*u[1]*v[3] + 3.0*u[2]*v[4] - 3.0*u[3]*v[1] - 3.0*u[4]*v[2] + u[5]*v[7] + u[6]*v[0] - u[7]*v[5], u[0]*v[7] - 3.0*u[1]*v[4] - 9.0*u[2]*v[3] - 9.0*u[3]*v[2] + 3.0*u[4]*v[1] - 3.0*u[5]*v[6] + 3.0*u[6]*v[5] + u[7]*v[0]);
}
