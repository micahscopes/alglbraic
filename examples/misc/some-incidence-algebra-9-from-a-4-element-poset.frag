...
uniform int FrameX; slider[1,1,9]
uniform int FrameY; slider[1,2,9]
uniform int FrameZ; slider[1,3,9]

// mutation indices (Lehmer or Lexicographic)
uniform int mutationA; slider[0,0,362880]
uniform int mutationB; slider[0,0,362880]
uniform int mutationC; slider[0,0,362880]
uniform int mutationD; slider[0,0,362880]
...
float[N] product(float u[N], float v[N]) {
    return float[N](1.0*pow(v[0], 2.0), v[0]*v[1] + v[1]*v[2], 1.0*pow(v[2], 2.0), v[0]*v[3] + v[1]*v[4] + v[3]*v[5], v[2]*v[4] + v[4]*v[5], 1.0*pow(v[5], 2.0), v[0]*v[6] + v[1]*v[7] + v[6]*v[8], v[2]*v[7] + v[7]*v[8], 1.0*pow(v[8], 2.0));
}


float norm(float u[N]) {
    return pow(pow(u[0], 2.0) + pow(u[1], 2.0) + pow(u[2], 2.0) + pow(u[3], 2.0) + pow(u[4], 2.0) + pow(u[5], 2.0) + pow(u[6], 2.0) + pow(u[7], 2.0) + pow(u[8], 2.0), 0.5);
}
...
float[N] loadParamsJuliaVect(out float u[N]){
    u[0] = JuliaVect1; u[1] = JuliaVect2; u[2] = JuliaVect3; u[3] = JuliaVect4; u[4] = JuliaVect5; u[5] = JuliaVect6; u[6] = JuliaVect7; u[7] = JuliaVect8; u[8] = JuliaVect9;
    return u;
}


float[N] loadParamsPosition(out float u[N]){
    u[0] = Position1; u[1] = Position2; u[2] = Position3; u[3] = Position4; u[4] = Position5; u[5] = Position6; u[6] = Position7; u[7] = Position8; u[8] = Position9;
    return u;
}
