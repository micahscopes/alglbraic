...
#info actions are random permutations, 3 dimensional real algebra with multiplication: (a1*b1+a3*b2+a1*b3) + (a2*b1+a1*b2+a3*b3)*E1 + (a3*b1+a2*b2+a2*b3)*E2
...
uniform int FrameZ; slider[1,3,3]
...
float[N] product(float u[N], float v[N]) {
    return float[N](u[0]*v[0] + u[0]*v[2] + u[2]*v[1], u[0]*v[1] + u[1]*v[0] + u[2]*v[2], u[1]*v[1] + u[1]*v[2] + u[2]*v[0]);
}
