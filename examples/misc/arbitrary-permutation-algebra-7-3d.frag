...
#info actions are random permutations, 7 dimensional real algebra with multiplication: (a1*b1+a5*b2+a4*b3+a5*b4+a6*b5+a1*b6+a2*b7) + (a2*b1+a2*b2+a2*b3+a4*b4+a2*b5+a4*b6+a4*b7)*E1 + (a3*b1+a3*b2+a7*b3+a1*b4+a4*b5+a7*b6+a6*b7)*E2 + (a4*b1+a1*b2+a1*b3+a7*b4+a5*b5+a3*b6+a3*b7)*E3 + (a5*b1+a7*b2+a5*b3+a3*b4+a1*b5+a2*b6+a5*b7)*E4 + (a6*b1+a4*b2+a6*b3+a6*b4+a3*b5+a5*b6+a1*b7)*E5 + (a7*b1+a6*b2+a3*b3+a2*b4+a7*b5+a6*b6+a7*b7)*E6
...
uniform int FrameX; slider[1,1,7]
uniform int FrameY; slider[1,2,7]
uniform int FrameZ; slider[1,3,7]

// sign involutions
uniform int flipperA; slider[0,0,128]
uniform int flipperB; slider[0,0,128]
uniform int flipperC; slider[0,0,128]
...
float[N] product(float u[N], float v[N]) {
    return float[N](u[0]*v[0] + u[0]*v[5] + u[1]*v[6] + u[3]*v[2] + u[4]*v[1] + u[4]*v[3] + u[5]*v[4], u[1]*v[0] + u[1]*v[1] + u[1]*v[2] + u[1]*v[4] + u[3]*v[3] + u[3]*v[5] + u[3]*v[6], u[0]*v[3] + u[2]*v[0] + u[2]*v[1] + u[3]*v[4] + u[5]*v[6] + u[6]*v[2] + u[6]*v[5], u[0]*v[1] + u[0]*v[2] + u[2]*v[5] + u[2]*v[6] + u[3]*v[0] + u[4]*v[4] + u[6]*v[3], u[0]*v[4] + u[1]*v[5] + u[2]*v[3] + u[4]*v[0] + u[4]*v[2] + u[4]*v[6] + u[6]*v[1], u[0]*v[6] + u[2]*v[4] + u[3]*v[1] + u[4]*v[5] + u[5]*v[0] + u[5]*v[2] + u[5]*v[3], u[1]*v[3] + u[2]*v[2] + u[5]*v[1] + u[5]*v[5] + u[6]*v[0] + u[6]*v[4] + u[6]*v[6]);
}
...
float[N] loadParamsJuliaVect(out float u[N]){
    u[0] = JuliaVect1; u[1] = JuliaVect2; u[2] = JuliaVect3; u[3] = JuliaVect4; u[4] = JuliaVect5; u[5] = JuliaVect6; u[6] = JuliaVect7; 
    return u;
}


float[N] loadParamsPosition(out float u[N]){
    u[0] = Position1; u[1] = Position2; u[2] = Position3; u[3] = Position4; u[4] = Position5; u[5] = Position6; u[6] = Position7; 
    return u;
}
