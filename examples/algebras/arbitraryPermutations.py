...
for k in range(dim-1):
    m = S.random_element().matrix()
    if(randomInversion):
        m.apply_map(lambda e: sgn(RR.random_element())*e)
    mats.append(m)
...
info = "actions are random permutations, %s dimensional real algebra with multiplication: " % dim
info = info + str(A*B)
...
name="arbitrary-permutation-algebra-%s" % dim
